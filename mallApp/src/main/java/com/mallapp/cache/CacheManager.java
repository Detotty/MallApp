...
public class CacheManager {
	...
	private CacheManager(Context applicationContext){
		...
		Log.d(AppCacheConstants.Tag, "[CacheManager]: Initializing new instance");
	}
	...
	public void write(String toWrite, String fileName) throws CacheTransactionException
	{
		...
		try {
			...
			out.write(toWrite);
			Log.d(AppCacheConstants.Tag, "[CacheManager]: Writing to " + mCacheDir + fileName);
		} catch (IOException e) {
			Log.d(AppCacheConstants.Tag, "[CacheManager]: Unsuccessful write to " + mCacheDir + fileName);
			e.printStackTrace();
			throw new CacheTransactionException(AppCacheConstants.writeExceptionAlert);
		}finally{
			if(out != null)
			{
				try {
	...
	public String readString(String fileName) throws CacheTransactionException
	{
		...
		try{
			...
			Log.d(AppCacheConstants.Tag, "[CacheManager]: Reading from " + mCacheDir + fileName);
			return readString;
		}catch(IOException e){
			Log.d(AppCacheConstants.Tag, "[CacheManager]: Unsuccessful read from " + mCacheDir + fileName);
			e.printStackTrace();
			throw new CacheTransactionException(AppCacheConstants.readExceptionAlert);
		}finally{
			if(in != null)
			{
				try {
	...
	public JSONObject readJSONObject(String fileName) throws CacheTransactionException{
		...
		try {
			...
			e.printStackTrace();
			Log.d(AppCacheConstants.Tag, "[CacheManager]: Successfully read the file " + mCacheDir + fileName + 
	...
	public void deleteFile(String fileName){
		Log.d(AppCacheConstants.Tag, "[CacheManager]: Deleting the file " + mCacheDir + fileName);
